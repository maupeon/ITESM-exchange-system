...
export class AuthenticationService {
    ...
    constructor(private http: HttpClient) {
        this.currentUserSubject = new BehaviorSubject<Teacher>(JSON.parse(localStorage.getItem('currentUser')));
        ...
        return this.http.post<any>(`https://api-itesm.azurewebsites.net/teachers/login/`, {_id, password})
            .pipe(map(user => {
                ...
                if (user && user.token) {
                    ...
                    localStorage.setItem('currentUser', JSON.stringify({mail:_id}));
